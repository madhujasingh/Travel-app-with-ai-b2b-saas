...
FIX (2025): _alignment_score now weights budget, mood/interests, and weather
equally (~27% each) instead of letting budget dominate.  Destination and other
signals fill the remaining ~19%.
"""
...
def build_package_catalog() -> pd.DataFrame:
    ...
    for di, destination in enumerate(DESTINATIONS):
        for vi, variant in enumerate(PACKAGE_VARIANTS):
            rows.append({
                ...
                "rating":           round(4.25 + variant["rating_bump"] + ((di + vi) % 3) * 0.08, 2),
                "review_count":     12 + di * 4 + vi * 7,
                ...
                "admin_id":         ADMIN_IDS[(di + vi) % len(ADMIN_IDS)],
...
def _alignment_score(
    ...
    """
    Compute an affinity score in [0, 1] for a (user, package) pair.

    WEIGHT PHILOSOPHY — all three primary axes are treated equally:
        budget   ~27 %   (was 30 %)
        mood     ~28 %   (was buried inside interests at ~35 % combined)
        weather  ~27 %   (was 18 %)
        other    ~18 %   destination + lane + group + season

    The old scheme let budget dominate, which caused mismatches like
    Amritsar (dry/warm) surfacing for rainy/couple searches.
    """
    score = 0.10   # small base so 0-signal pairs still get a tiny value

    # ── Mood match  (28 %) ────────────────────────────────────────────────────
    # Full credit on direct hit; partial credit for any overlap in the profile.
    if mood:
        profile_moods = [m.strip().lower() for m in str(package["mood_profile"]).split(",")]
        if mood.lower() in profile_moods:
            score += 0.28
        elif any(mood.lower() in m or m in mood.lower() for m in profile_moods):
            score += 0.10   # partial / synonym overlap

    # ── Weather match  (27 %) ─────────────────────────────────────────────────
    if weather_pref:
        pkg_weather = str(package["weather_profile"]).lower().strip()
        # Direct hit
        if weather_pref == pkg_weather:
            score += 0.27
        # Compatible pairs (e.g. user wants "rainy", package is "warm" → partial)
        elif _weather_compatible(weather_pref, pkg_weather):
            score += 0.12
        # Mismatch — no points; the absence is the penalty

    # ── Budget match  (27 %) ─────────────────────────────────────────────────
    pkg_price = int(package["price_inr"])
    if budget >= pkg_price:
        # Within budget: scale reward by how close to ideal (not just "yes/no")
        ratio  = pkg_price / max(budget, 1)           # 1.0 = perfect use of budget
        closeness = 1.0 - abs(ratio - 0.85)           # sweet-spot around 85 % of budget
        score += max(0.12, min(0.27, 0.27 * (0.5 + closeness)))
    elif budget >= pkg_price * 0.85:
        score += 0.08   # slightly over budget but reachable stretch
    ...
    # ── Trip lane match  (5 %) ───────────────────────────────────────────────
    if trip_lane and trip_lane.upper() == str(package["market"]).upper():
        score += 0.05
    ...
    # ── Season / month fit  (2 %) ────────────────────────────────────────────
    if trip_month and trip_month in str(package["best_months"]):
        score += 0.02

    return float(np.clip(score, 0.0, 1.0))


def _weather_compatible(user_pref: str, pkg_weather: str) -> bool:
    """Return True when the package weather is a reasonable fit for the preference."""
    compat = {
        "warm":      {"hot", "mild"},
        "rainy":     {"warm", "humid", "mild"},
        "cool":      {"temperate", "mild", "cold"},
        "cold":      {"cool"},
        "temperate": {"warm", "cool", "mild"},
    }
    return pkg_weather in compat.get(user_pref, set())
...
def build_interactions(packages_df: pd.DataFrame, interaction_count: int) -> pd.DataFrame:
    ...
    for interaction_id in range(1, interaction_count + 1):
        ...
        eligible = packages_df[packages_df["market"].str.lower() == trip_lane]
        if rng.random() < 0.66:
            package            = eligible.sample(1, random_state=int(rng.integers(0, 1_000_000))).iloc[0]
