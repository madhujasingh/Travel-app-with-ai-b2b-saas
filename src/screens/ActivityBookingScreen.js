...
const ActivityBookingScreen = ({ route, navigation }) => {
  ...
  // HotelBeds sometimes returns a ready-made supplier voucher (barcode/QR
  // direct-connection activities) - per their docs that PDF/HTML MUST be
  // handed to the customer as-is, never regenerated by us, since it carries
  // the supplier's own redemption identification. Only when they don't
  // provide one do we generate our own from the booking data.
  const downloadVoucher = async () => {
    setDownloadingVoucher(true);
    try {
      const resolveResponse = await fetch(
        `${API_CONFIG.BASE_URL}/activities/bookings/en/${booking.reference}/voucher`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      const resolveData = await resolveResponse.json();
      if (!resolveResponse.ok) {
        throw new Error(parseActivitiesError(resolveData, 'Unable to load this voucher right now.'));
      }

      if (resolveData?.hasSupplierVoucher && resolveData?.url) {
        await WebBrowser.openBrowserAsync(resolveData.url);
        return;
      }

      const pdfResponse = await fetch(
        `${API_CONFIG.BASE_URL}/activities/bookings/en/${booking.reference}/voucher/pdf`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (!pdfResponse.ok) {
      ...
      const file = new FileSystem.File(FileSystem.Paths.cache, `activity-voucher-${booking.reference}.pdf`);
      ...
      <SafeAreaView style={styles.container}>
        ...
        <ScrollView contentContainerStyle={styles.confirmedState}>
          ...
          {booking.status !== 'CANCELLED' && (
            <TouchableOpacity style={styles.secondaryButton} onPress={downloadVoucher} disabled={downloadingVoucher}>
              {downloadingVoucher ? (
                <ActivityIndicator color={Colors.primary} />
              ) : (
